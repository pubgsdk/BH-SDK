...
public:
	...
	{
		const auto nameToFindLen = StringUtils::Len(nameToFind);
		if (nameToFindLen == -1) return false;
		...
		while (It < End)
		{
			...
			{
				if (nameToFindLen == Len && StringUtils::CompareSub(Entry->GetName(), nameToFind, 0, Len))
				{
					out = { entryHandle.ToEntryId(), Entry->GetName(), Len };
					return true;
				}
...
struct FName
{
	...
	{
		auto* nameEntry = GetGlobalNames().GetById(ComparisonIndex.Value);
		if(!nameEntry) { return -1; }
		int len = 0;
		const auto namePtr = nameEntry->GetName(&len);
		return StringUtils::Copy(nameOut, namePtr, len);
	}
